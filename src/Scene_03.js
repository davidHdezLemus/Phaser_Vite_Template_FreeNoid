...
export default class Scene_03 extends Phaser.Scene {
  constructor() {
    super({ key: "Scene_03" });
    this.money = null;
    this.coins = 0;
  ...
  init(data) {
    ...
    this.livesCount = data.lives !== undefined ? data.lives : 3;
  }

  preload() {
    // Load images and audio assets
    this.load.image("background_02", "assets/background02.png");
  ...
  create() {
    ...
    const skyTile02 = map.addTilesetImage("background02", "background_02");
    const tilesetTile = map.addTilesetImage("bricks", "blocks");
    map.createLayer("background02", skyTile02);
  ...
  createBlocks() {
    ...
    for (let i = 0; i < totalBlocks; i++) {
      ...
      const blockType = Phaser.Math.Between(1, 10) * 2; // Use even blocks
  ...
  createLives() {
    this.lives = [];
    for (let i = 0; i < 3; i++) {
      const texture = i < this.livesCount ? 'life_02' : 'life_01';
      const life = this.add.sprite(165 + (i * 20), 24, 'life', texture);
      life.setScale(1.5);
